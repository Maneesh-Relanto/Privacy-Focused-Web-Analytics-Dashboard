...
export async function testAuthentication() {
  ...
  try {
    ...
    assertEquals(registerResponse.data.user?.email, email, 'Email should match');
    logSuccess(`✅ User registration successful (${registerResponse.timing}ms)\n`);
    ...
    assertEquals(duplicateResponse.status, 400, 'Duplicate registration should fail with 400');
    logSuccess(`✅ Duplicate registration correctly rejected (${duplicateResponse.timing}ms)\n`);
    ...
    assertExists(loginResponse.data.accessToken, 'Access token should be returned');
    ...
    assertEquals(invalidLoginResponse.status, 401, 'Invalid login should fail with 401');
    logSuccess(`✅ Invalid login correctly rejected (${invalidLoginResponse.timing}ms)\n`);
    ...
    assertEquals(protectedResponse.status, 200, 'Protected route should be accessible with token');
    logSuccess(`✅ Protected route access granted (${protectedResponse.timing}ms)\n`);
    ...
    assertEquals(noTokenResponse.status, 401, 'Protected route without token should fail with 401');
    logSuccess(`✅ Protected route correctly blocked (${noTokenResponse.timing}ms)\n`);
    ...
    assertEquals(invalidTokenResponse.status, 401, 'Invalid token should fail with 401');
    logSuccess(`✅ Invalid token correctly rejected (${invalidTokenResponse.timing}ms)\n`);
