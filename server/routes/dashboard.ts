  ...
  try {
    ...
    const generateDataPoints = () => {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        points.push({
          date: isHourly ? date.toISOString().slice(11, 16) : date.toISOString().slice(0, 10),
 ...
 */
router.get('/top-pages', authMiddleware, async (req: Request, res: Response) => {
  try {
    const topPages = {
      pages: [
 ...
 */
router.get('/referrers', authMiddleware, async (req: Request, res: Response) => {
  try {
    const referrers = {
      referrers: [
...
router.get('/devices', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const desktop = Math.floor(total * 0.40)
    const tablet = total - mobile - desktop

    const devices = {
      devices: [
        { type: 'mobile' as const, count: mobile, percentage: (mobile / total) * 100 },
        { type: 'desktop' as const, count: desktop, percentage: (desktop / total) * 100 },
        { type: 'tablet' as const, count: tablet, percentage: (tablet / total) * 100 },
      ],
    }
...
router.get('/locations', authMiddleware, async (req: Request, res: Response) => {
  try {
    const locations = {
      locations: [
        { country: 'United States', city: 'San Francisco, CA', visitors: 3421, percentage: 26.6 },
        { country: 'United States', city: 'New York, NY', visitors: 2156, percentage: 16.8 },
        { country: 'India', city: 'Bangalore', visitors: 1843, percentage: 14.3 },
        { country: 'United Kingdom', city: 'London', visitors: 1234, percentage: 9.6 },
        { country: 'Germany', city: 'Berlin', visitors: 987, percentage: 7.7 },
      ],
    }
